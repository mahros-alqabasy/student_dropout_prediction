...
def submit():
    if request.method == 'POST':
        ...
        # Step 2: Extract features from the form
        # Here we're assuming the form has fields named 'feature1', 'feature2', etc.
        form_data = request.form
        feature_values = [float(form_data['feature1']), float(form_data['feature2']), ...]  # Replace with actual form field names

        # Append the new data point to your dataset
        new_data_point = np.array(feature_values).reshape(1, -1)
        df_features = df[['feature1', 'feature2', ...]].values  # Replace with actual feature columns
        combined_data = np.vstack([df_features, new_data_point])

        # Step 3: Perform Fuzzy C-Means clustering
        cntr, u, u0, d, jm, p, fpc = fuzz.cluster.cmeans(
            combined_data.T, 2, m=2, error=0.005, maxiter=1000
        )

        # Step 4: Retrieve cluster membership for the new data point
        memberships = u[:, -1].tolist()  # The last entry is the new data point
        print(memberships)  # This should output the membership values for each cluster

        # Step 5: Return results
        return redirect(
            url_for('result', valr=prediction[0], sname=form_data['name'], memberships=','.join(map(str, memberships))))

    return render_template('form.html')
@app.route("/result/<valr>/<sname>")
def result(valr, sname):
    memberships_str = request.args.get('memberships', '')
    memberships = [float(m) for m in memberships_str.split(',')] if memberships_str else []
    print("Retrieved memberships:", memberships)
    return render_template("submit.html", pred=valr, name=sname, memberships=memberships)
